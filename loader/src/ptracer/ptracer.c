...
static bool inject_tango(int pid, const char *lib_path, uint32_t libc_init_target, uint32_t libc_init_got_slot) {
  ...
  if (!ptrace_poke_u32(pid, (uintptr_t)libc_init_got_slot, libc_init_target)) { LOGW("Failed to restore GOT at 0x%x", libc_init_got_slot); }
