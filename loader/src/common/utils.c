...
void tracee_skip_syscall(int pid) {
  ...
  /* no idea why this works, but touching it feels dangerous 
  TODO: figure out why this works before someone asks
  hahaha LOL
  */
  #if defined(__aarch64__)
    int sysnr = -1;
...
void wait_for_trace(int pid, int *status, int flags) {
  while (1) {
    ...
    /* We'll fork there. This will signal SIGCHLD. We just ignore and continue
               to avoid blocking */
    if (WIFSTOPPED(*status) && WSTOPSIG(*status) == SIGCHLD) {
      ...
      /* Return the status to the caller instead of killing the tracer */
      return;
    }
